...
1. Use `checkenv` to check everything from the status of Docker process, currently running servers, status of essential services much faster than the official `checkServices` (keep the script up to date for any possible changes), and also if the currently running Onshape server is actually 'functional'. Similarly, use functions like `startenv` and `stopenv` to start or stop all of them automatically. There's also a `fixenv` for fixing services or servers that are not currently running. 
